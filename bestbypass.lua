...
local a = loadstring(game:HttpGet("https://raw.githubusercontent.com/AbstractPoo/Main/main/Notifications.lua", true))()
...
local s = r:FindFirstChild("SayMessageRequest", true)
local t
t =
    hookmetamethod(
    s,
    ...
    function(self, ...)
        ...
        local v = {...}
        if self == s and u == "FireServer" then
            local i = v[1]
            local w = i:split(" ")
            local x = ""
            for y, z in next, w do
                for y, A in next, q do
                    if z:lower() == y then
                        if z:upper() ~= z then
                            i = i:gsub(z, A)
                            x = i .. " ng"
...
if not LOADED then
    a:message {
        ...
        Description = "FIRSTLY DISCOVERED WAS BY tav#8870. Bypasser was done by Decayed#5038 and AnthonyIsHere on V3rm. Modified by daddysyn, added sum words bc there not all words *sobs*. tayski",
        Icon = 6023426926
    }
...
a:notify {
    ...
    Dismiss = {Text = "", Callback = function()
        a:message {Title = "<font color='rgb(240,248,255)'>Warning</font>", Description = "omg ty for joining", Icon = 10952973359}
...
wait(10)
a:message {Title = "<font color='rgb(240,248,255)'>ily</font>", Description = "ty for using best bypass, info : stolen from sengery but i a bit edited the script", Icon = 10952973359}
pcall(
    function()
        getgenv().LOADED = true
    end
